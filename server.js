...
3. 교재에 없는 내용이면: "외부지문"
4. **간접연계 판별 (매우 중요)**: 시험지의 지문이 교재의 어떤 지문과 완전히 동일하지는 않지만, 아래 조건 중 하나라도 해당하면 간접연계로 판별:
   - 소재/주제가 같지만 지문 내용이 다르게 재작성된 경우
   - 교재 지문의 핵심 개념을 활용하되 문장을 새로 쓴 경우
   - 같은 연구/실험/사례를 다루지만 표현이 바뀐 경우
   → 이 경우: "고1 24년 3월 37번(간접연계)" 형태로 표기 (외부지문이 아님!)
5. 교재의 어떤 지문과도 소재/주제 연관이 전혀 없는 경우만: "외부지문"
