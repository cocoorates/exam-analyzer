...
- 문법 → "어법판단"
- 어휘 → "문맥어휘"
- 빈칸에 들어갈 연결어(접속사/부사)를 고르는 문제 (선지가 however, for example, therefore, in addition 등 연결어로만 구성) → "연결어"
- 위 연결어 문제를 제외한 나머지 빈칸 문제 → "빈칸추론"
- 순서 → "순서배열"
- 삽입 → "문장삽입"
- 요약 → "내용요약"
...
**직접연계 (교재에 있는 지문)**: 시험지 지문이 교재에 수록된 지문과 동일하거나 거의 동일한 경우 (같은 글을 그대로 가져온 것이 명확한 경우) → 원본 출처 그대로 표기 (예: "고2 24년 6월 29번")

**외부지문**: 시험지 지문이 교재에 수록된 어떤 지문과도 일치하지 않는 경우 (교재에 없는 글) → "외부지문" 으로 표기

**외부지문 + 간접연계**: 교재에 없는 지문(외부지문)이지만, 교재에 수록된 특정 지문과 '소재'나 '중심내용'이 겹치는 경우 (예: 둘 다 "꿀벌의 집단지성"을 다루지만 완전히 다른 글) → "외부지문(고2 24년 3월 37번-간접연계)" 형태로 표기

참고: 교과서 본문 지문이면 "교과서 2과" 형태로 표기

중요: 간접연계는 매우 드문 경우입니다. 단순히 같은 모의고사 출처라는 이유만으로 간접연계로 판단하지 마세요. 반드시 시험지 지문의 소재/중심내용과 교재 지문의 소재/중심내용이 실제로 겹치는지 꼼꼼히 비교한 후 판단하세요.
