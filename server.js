...
**1단계 - 직접연계**: 시험지 지문과 교재 지문을 비교했을 때 누가 봐도 같은 글이면 (단어/문장이 90% 이상 동일) → 원본 출처 그대로 표기 (예: "고1 24년 6월 29번")

**2단계 - 간접연계 (매우 중요!)**: 시험지 지문이 교재의 어떤 지문과 비교했을 때 50% 이상이 다른 단어/표현으로 바뀌어 있어서 누가 봐도 "같은 지문"이라고 할 수 없지만, 잘 보면 소재나 주제가 겹치는 경우. 즉 교재 지문의 소재를 가져다가 내용을 완전히 새로 쓴 경우.
→ "고1 24년 3월 37번(간접연계)" 형태로 표기

**3단계 - 외부지문**: 교재의 어떤 지문과도 소재/주제가 전혀 겹치지 않는 완전히 새로운 지문
→ "외부지문"

참고: 교과서 지문이면 "교과서 2과" 형태로 표기
